...
CC_MODEL_FOR_SEQUENCE_CLASSIFICATION_MAPPING = OrderedDict(
    [
        (AlbertConfig, AlbertCcForSequenceClassification),
        (BertConfig, BertCcForSequenceClassification),
    ]
)
...
MODEL_FOR_SEQUENCE_AND_TOKEN_CLASSIFICATION_MAPPING = OrderedDict(
    [
        (AlbertConfig, AlbertForSequenceAndTokenClassification),
        (BertConfig, BertForSequenceAndTokenClassification),
    ]
)

CC_MODEL_FOR_SEQUENCE_AND_TOKEN_CLASSIFICATION_MAPPING = OrderedDict(
    [
        (AlbertConfig, AlbertCcForSequenceAndTokenClassification),
        (BertConfig, BertCcForSequenceAndTokenClassification),
    ]
)
...
class AutoModelForSequenceClassification(object):
    ...
    def from_config(cls, config):
        ...
        try:
            if config.cc:
                for config_class, model_class in CC_MODEL_FOR_SEQUENCE_CLASSIFICATION_MAPPING.items():
                    if isinstance(config, config_class):
                        return model_class(config)
            else:
        ...
        raise ValueError(
            "Unrecognized configuration class {} for this kind of AutoModel: {}.\n"
            "Model type should be one of {} or one of {}.".format(
                config.__class__,
                cls.__name__,
                ", ".join(c.__name__ for c in MODEL_FOR_SEQUENCE_CLASSIFICATION_MAPPING.keys()),
                ", ".join(c.__name__ for c in CC_MODEL_FOR_SEQUENCE_CLASSIFICATION_MAPPING.keys()),
            )
        )
...
class AutoModelForSequenceAndTokenClassification(object):
    r"""
        :class:`~transformers.AutoModelForSequenceAndTokenClassification` is a generic model class
        that will be instantiated as one of the sequence and token classification model classes of the library
        when created with the `AutoModelForSequenceAndTokenClassification.from_pretrained(pretrained_model_name_or_path)`
        class method.
    ...
    def __init__(self):
        raise EnvironmentError(
            "AutoModelForSequenceAndTokenClassification is designed to be instantiated "
            "using the `AutoModelForSequenceAndTokenClassification.from_pretrained(pretrained_model_name_or_path)` or "
            "`AutoModelForSequenceAndTokenClassification.from_config(config)` methods."
        )
    ...
    def from_config(cls, config):
        ...
        Args:
            config (:class:`~transformers.PretrainedConfig`):
                ...
                - isInstance of `albert` configuration class: :class:`~transformers.AlbertModelForSequenceAndTokenClassification` (ALBERT model)
                - isInstance of `bert` configuration class: :class:`~transformers.BertModelForSequenceAndTokenClassification` (Bert model)


        Examples::

            config = BertConfig.from_pretrained('bert-base-uncased')    # Download configuration from S3 and cache.
            model = AutoModelForSequenceAndTokenClassification.from_config(config)  # E.g. model was saved using `save_pretrained('./test/saved_model/')`
        """
        try:
            if config.cc:
                for config_class, model_class in CC_MODEL_FOR_SEQUENCE_AND_TOKEN_CLASSIFICATION_MAPPING.items():
                    if isinstance(config, config_class):
                        return model_class(config)
            else:
                for config_class, model_class in MODEL_FOR_SEQUENCE_AND_TOKEN_CLASSIFICATION_MAPPING.items():
                    if isinstance(config, config_class):
                        return model_class(config)
        except:
            for config_class, model_class in MODEL_FOR_SEQUENCE_AND_TOKEN_CLASSIFICATION_MAPPING.items():
        ...
        raise ValueError(
            ...
            "Model type should be one of {} or one of {}.".format(
                config.__class__,
                cls.__name__,
                ", ".join(c.__name__ for c in MODEL_FOR_SEQUENCE_AND_TOKEN_CLASSIFICATION_MAPPING.keys()),
                ", ".join(c.__name__ for c in CC_MODEL_FOR_SEQUENCE_AND_TOKEN_CLASSIFICATION_MAPPING.keys()),
            )
        )
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        Examples::
            ...
            model = AutoModelForSequenceAndTokenClassification.from_pretrained('bert-base-uncased')    # Download model and configuration from S3 and cache.
            model = AutoModelForSequenceAndTokenClassification.from_pretrained('./test/bert_model/')  # E.g. model was saved using `save_pretrained('./test/saved_model/')`
            assert model.config.output_attention == True
            # Loading from a TF checkpoint file instead of a PyTorch model (slower)
            config = AutoConfig.from_json_file('./tf_model/bert_tf_model_config.json')
            model = AutoModelForSequenceAdnTokenClassification.from_pretrained('./tf_model/bert_tf_checkpoint.ckpt.index', from_tf=True, config=config)
